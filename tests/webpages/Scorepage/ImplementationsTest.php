    ...
    <body>
        ...
        <table class="scoretable">
            ...
            <tbody id = "scoreboard">
                ...
                #$retDig = '7777';
                //This line fires the retrieveDigits function which sets ranks->currentDigits
                $ranks->retrieveDigits($ranks->dbconn, $retDig);
                //this line retrieves the now set currentDigits rdtestvar stands for "retrieve digits test variable"
                $rdtestvar = $ranks->currentDigits;
                //this line echoes the status message
                echo $ranks->retrieveDigits($ranks->dbconn, $retDig);
                //this if statement fires if the digits have been inserted into the table previously "retrieve digits test array"
                if ($rdtestvar->num_rows > 0) {
                ...
                $retScore ="test7";
                //uncomment to pull a nonexistant name, if this name exists, find one that doesn't or create a dummy copy of the database
                #$retScore = 'zyuyuiuyi';
                //This line fires the retrieveScore function which sets ranks->currentDigits
                $ranks->retrieveUserScore($ranks->dbconn, $retScore);
                //this line retrieves the now set currentDigits rstestvar stands for "retrieve score test variable"
                $rstestvar = $ranks->currentScore;
                //this line echoes the status message
                echo $ranks->retrieveUserScore($ranks->dbconn, $retScore);
                //this if statement fires if the digits have been inserted into the table previously "retrieve digits test array"
                if ($rstestvar->num_rows > 0) {
                    $rstestarr=mysqli_fetch_array($rstestvar);
                    echo " ".$rstestarr["user_score"];}
                #*/
                ##############
                ##############
                #/*
                //Test for prevention of existing user input
                $newuser = "'test1'";
                $newpw = "password";
                $ranks->addNewUser($ranks->dbconn, $newuser, $newpw);
                //test for adding new user
                $newuser = "'test30'";
                $newpw = "V1t7pY";
                $ranks->addNewUser($ranks->dbconn, $newuser, $newpw);
                //delete test
                #$ranks->deleteUser($ranks->dbconn, $newuser, $newpw);
                #*/
